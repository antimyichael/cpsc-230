...
def top_3_movies():
    movies = ""
    movies = (input("Input your top three movies\n"))
    movies = movies.split(", ")
    if len(movies) != 3:
        print("Failed to provide three movies.")
        top_3_movies()
    mov1, mov2, mov3 = movies
...
def add_hotel_rating(ratings, new_rating):
    ...
    avg = ("%.2f" % ((sum(ratings))/(len(ratings))))
    print(f"Average Rating: {avg}")
average_ratings = [7, 8.1]
add_hotel_rating(average_ratings, 6)